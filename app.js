...
async function admin(){
  ...
  async function loadNews(){
    ...
    $$('[data-edit-news]').forEach(b=>b.onclick=()=>{const n=data.find(x=>x.id===b.dataset.editNews);$('#news-id').value=n.id;$('#news-title').value=n.title;$('#news-body').value=n.body;$('#news-published').checked=n.published;$('#news-title').focus()});
  ...
  async function loadEvents(){
    const {data,error}=await c.from('events').select('*').order('event_date');
    ...
    $$('[data-edit-event]').forEach(b=>b.onclick=()=>{const v=data.find(x=>x.id===b.dataset.editEvent);$('#event-id').value=v.id;$('#event-title').value=v.title;$('#event-date').value=v.event_date;$('#event-time').value=v.event_time?.slice(0,5)||'';$('#event-location').value=v.location||'';$('#event-description').value=v.description||'';$('#event-title').focus()});
    $$('[data-del-event]').forEach(b=>b.onclick=async()=>{if(confirm('Delete this event?')){const {error}=await c.from('events').delete().eq('id',b.dataset.delEvent);if(error)message(error.message,'error');else{message('Event deleted.');loadEvents()}}})
  }
